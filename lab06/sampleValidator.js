...
var SampleValidator = (function () {
    ...
    function validateEmail(email, messages) {
        ...
        if(!pattern.test(email)){
            messages.push('The email address '+ email + ' is not valid');
        }
    }

    function validatePostCode(code, message){
        var pattern = /(^[0-9]{4}$)/g;
        if(!pattern.test(code)){
            message.push("Postal code is incorrectly intered");
        }
    }


    function addressNotEmpty(messages){
        var addressElements = ['deliveryName', 'deliveryAddress1', 'deliveryCity'];
    }
    //lab stuff end
    ...
    function validateCheckout() {
        var messages, cardType, cardNumber, cardMonth, cardYear, cardValidation, errorHTML, email, postal;
        ...
        postal = document.getElementById('deliveryPostcode').value
        validatePostCode(postal, messages);
